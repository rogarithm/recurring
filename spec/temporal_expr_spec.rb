...
describe "temporal expression" do
  ...
  it "주어진 day_num(Date 객체가 표현하는 날짜가 해당 달에서 몇번째 날인지를 표현하는 값)이 몇번째 주에 속하는지 알 수 있다" do
    ignore1, ignore2 = 1, 1
    dim = DayInMonth.new(ignore1, ignore2)

    (1..7).each {|first_week_day|
      expect(dim.week_in_month first_week_day).to eq(1)
    }
    (8..14).each {|second_week_day|
      expect(dim.week_in_month second_week_day).to eq(2)
    }
    (15..21).each {|third_week_day|
      expect(dim.week_in_month third_week_day).to eq(3)
    }
    (22..28).each {|fourth_week_day|
      expect(dim.week_in_month fourth_week_day).to eq(4)
    }
    (29..31).each {|fifth_week_day|
      expect(dim.week_in_month fifth_week_day).to eq(5)
    }
  end
  ...
  it "주어진 날짜에서, 해당 달의 마지막 날까지 남은 날의 수를 구할 수 있다" do
    ...
    dim = DayInMonth.new(ignore1, ignore2)
    monday = Date.new(2024, 11, 11)
    expect(dim.days_left_in_month monday).to eq(19)
  end
end